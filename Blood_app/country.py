...
# Current Bangladesh administrative names. Updated September 2026.
country_data = {
    " ":
    {
        " ":[" "],
	...
	"Mymensingh":
	{
		...
		"Mymensingh": [" ","Trishal", "Dhobaura", "Fulbaria", "Gafargaon", "Gauripur", "Haluaghat", "Ishwarganj", "Mymensingh Sadar", "Muktagachha", "Nandail", "Phulpur", "Bhaluka", "Tarakanda"],
	...
	{
        " ":[" "],
		"Bogura": [" ","Adamdighi", "Bogura Sadar", "Dhunat", "Dhupchanchia", "Gabtali", "Kahaloo", "Mokamtola", "Nandigram", "Sariakandi", "Shajahanpur", "Sherpur", "Shibganj", "Sonatola"],
	...
	"Rangpur":
	{
		...
		"Thakurgaon": [" ","Baliadangi", "Bhulli", "Haripur", "Pirganj", "Ranisankail", "Ruhia", "Thakurgaon Sadar"]
	},
	"Sylhet":
	{
        " ":[" "],
		...
		"Sunamganj": [" ","Bishwamvarpur", "Chhatak", "Derai", "Dharmapasha", "Dowarabazar", "Jagannathpur", "Jamalganj", "Madhyanagar", "Shantiganj", "Shalla", "Sunamganj Sadar", "Tahirpur"],
...
# Remove the blank placeholders retained by the original website's dropdowns.
country_data = {
    division.strip(): {
        district.strip(): [upazila.strip() for upazila in upazilas if upazila.strip()]
        for district, upazilas in districts.items()
        if district.strip()
    }
    for division, districts in country_data.items()
    if division.strip()
}
...
#             address = f"{upazila}, {district}, {division}"
#             if(get_lat_lon(address)=="No results found"):
#                 print(f"here is the address {address}")
